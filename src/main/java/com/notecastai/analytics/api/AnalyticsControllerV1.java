...
public class AnalyticsControllerV1 {
    ...
    @Operation(
            ...
            description = "Get analytics statistics for the current user over a specified period"
    )
    @ApiResponses({
    ...
    public AnalyticsStatsResponse getStats(
            @Parameter(description = "Analytics period (WEEK, MONTH, YEAR, ALL)")
            @RequestParam(defaultValue = "MONTH") AnalyticsPeriod period
    ) {
        return analyticsService.getStats(period);
    }
}