...
import com.notecastai.note.api.mapper.NoteCommandMapper;
...
public class NoteControllerV1 {
    ...
    private final NoteCommandMapper commandMapper;
    ...
    public NoteDTO create(@Valid @RequestBody CreateNoteRequest request) {
        return noteService.create(commandMapper.toCommand(request));
    }
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Combined note created successfully"),
    ...
    public NoteDTO combine(@Valid @RequestBody NoteCombineRequest request) {
        return noteService.combine(commandMapper.toCommand(request));
    }
    ...
    public NoteDTO update(
        ...
        return noteService.updateManual(id, commandMapper.toCommand(request));
    }
    ...
    public NoteDTO formatNoteKnowledgeBase(
            @Parameter(description = "Note ID", required = true)
            @PathVariable Long id,
            @Valid @RequestBody NoteKnowledgeFormatRequest request
    ) {
        return noteService.formatNoteKnowledgeBase(id, commandMapper.toCommand(request));
    }
    ...
    public NoteDTO adjustNote(
            @Parameter(description = "Note ID", required = true)
            @PathVariable Long id,
            @Valid @RequestBody FormatNoteRequest request
    ) {
        return noteService.formatNote(id, commandMapper.toCommand(request));
    }
    ...
    public NoteQuestionResponse askQuestion(
        ...
        return noteService.askQuestion(noteId, commandMapper.toCommand(request));
    }
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Note cloned successfully"),
            @ApiResponse(responseCode = "404", description = "Note not found", content = @Content),
