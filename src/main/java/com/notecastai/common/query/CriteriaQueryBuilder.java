...
public class CriteriaQueryBuilder<T extends Serializable> {
    ...
    private long countInternal(Ctx<T> ctx) {
        // Criteria predicates are bound to the Root they were created from, so we have
        // to rebuild them against this query's own Root before using them here.
        CriteriaQuery<Long> cq = ctx.cb.createQuery(Long.class);
        Root<T> root = cq.from(entityClass);
        cq.select(distinct ? ctx.cb.countDistinct(root) : ctx.cb.count(root));
