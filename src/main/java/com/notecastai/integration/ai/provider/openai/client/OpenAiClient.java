...
import static com.notecastai.common.exception.TechnicalException.Code.AI_SERVICE_ERROR;
...
public class OpenAiClient {
    ...
    public OpenAiTranscriptionResponse transcribe(OpenAiTranscriptionRequest request) {
        try {
            ...
            log.warn("Global AI rate limit exceeded for OpenAI transcription");
    ...
    public OpenAiSpeechResponse createSpeech(OpenAiSpeechRequest request, MediaType acceptType) {
        try {
            RateLimiter.waitForPermission(globalTtsRateLimiter);
        } catch (RequestNotPermitted e) {
            log.warn("Global TTS rate limit exceeded for OpenAI speech");
            throw TechnicalException.of(AI_SERVICE_ERROR)
                    .with("provider", "OpenAI")
                    .with("error", "TTS service rate limit exceeded. Please try again shortly.")
                    .build();
        }
        try {
