package com.notecastai.integration.storage.minio;
...
/**
 * StorageService implementation backed by MinIO (S3-compatible) using the
 * AWS SDK v2 S3 client with path-style access.
 */
@Slf4j
@Service
public class MinioStorageService implements StorageService {
    ...
    public CompletableFuture<String> putAsync(String key, InputStream data, long size, String ct) {
        log.info("Starting async MinIO upload: {}", key);
        try {
            s3.putObject(
                    PutObjectRequest.builder().bucket(bucket).key(key).contentType(ct).build(),
                    RequestBody.fromInputStream(data, size)
            ...
            getCache.invalidate(key);
            log.info("Async MinIO upload completed: {}", key);
            return CompletableFuture.completedFuture(key);
        } catch (Exception e) {
            log.error("Async MinIO upload failed: {}", key, e);
    ...
    public String presignedPut(String key, String ct) {
        ...
        try {
            ...
            throw TechnicalException.of(TechnicalException.Code.MINIO_ERROR)
                    .with("key", key)
                    .cause(ex)
                    .build();
        }
